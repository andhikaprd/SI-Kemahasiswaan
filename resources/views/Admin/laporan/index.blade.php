        ...
        <div class="col-md-3">
            <div class="card shadow-sm border-0 text-center text-primary">
                <div class="card-body">
                    ...
                    <h5 class="card-title">{{ $totalLaporan ?? 0 }}</h5>
                        ...
                        @forelse ($laporans as $laporan)
                            <tr>
                                <td>
                                    ...
                                    <small class="text-muted">Pembuat: {{ $laporan->pembuat ?? 'N/A' }}</small>
                                </td>
                                <td>{{ $laporan->periode ?? 'N/A' }}</td>
                                <td><span class="badge bg-primary">{{ $laporan->kategori ?? 'Umum' }}</span></td>
                                <td>{{ $laporan->created_at->format('d M Y') }}</td>
                                <td><span class="badge bg-success">{{ Str::ucfirst($laporan->status ?? 'Final') }}</span></td>
                                <td class="text-center">
                                    <a href="{{ route('admin.laporan.edit', $laporan->id) }}" class="btn btn-sm btn-outline-secondary me-2">Edit</a>
