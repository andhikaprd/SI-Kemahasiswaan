...
<div class="bg-white shadow rounded-xl p-6 max-w-3xl mx-auto">
    ...
    <form method="POST" action="{{ route('kaprodi.masalah_mahasiswa.store') }}">
        ...
        {{-- Mahasiswa (dropdown + input manual) --}}
        <div class="mb-3">
            <label class="block font-medium mb-1">Mahasiswa</label>
            <input list="mahasiswaList" name="mahasiswa_id" 
        ...
        <div class="mt-3">
            ...
            <input type="text" name="jenis_masalah" 
                   class="border rounded-lg w-full px-3 py-2" 
                   value="{{ old('jenis_masalah') }}">
        </div>

        {{-- Status Peringatan --}}
        <div class="mt-3">
            <label class="block font-medium mb-1">Status Peringatan</label>
            <select name="status_peringatan" 
                    class="border rounded-lg w-full px-3 py-2">
        ...
        <div class="mt-3">
            ...
            <input type="date" name="laporan_terakhir" 
                   class="border rounded-lg w-full px-3 py-2" 
                   value="{{ old('laporan_terakhir') }}">
        </div>
        ...
        <div class="mt-3">
            ...
            <textarea name="keterangan" rows="3" 
                      class="border rounded-lg w-full px-3 py-2">{{ old('keterangan') }}</textarea>
        </div>
